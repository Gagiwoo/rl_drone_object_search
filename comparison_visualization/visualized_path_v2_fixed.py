...
def calculate_tp_fp(detected_weeds, ground_truth_weeds):
    """
    True Positive와 False Positive 계산

    Returns:
        tp_count (int): 실제로 찾은 잡초 개수
        fp_count (int): 잘못 탐지한 개수
        fn_count (int): 놓친 잡초 개수
    """
    # Detected set 생성
    detected_set = set()
    ...
    # Ground truth set 생성
    gt_set = set()
    if ground_truth_weeds is not None and len(ground_truth_weeds) > 0:
        for weed in ground_truth_weeds:
            if isinstance(weed, (list, np.ndarray)):
                gt_set.add(tuple(weed))
            else:
                gt_set.add(weed)

    # TP, FP, FN 계산
    true_positives = detected_set & gt_set  # 교집합
    false_positives = detected_set - gt_set  # 잘못 탐지
    false_negatives = gt_set - detected_set  # 놓친 것
...
def collect_episode_data(model_path, config_file, n_episodes=5, seed=42, model_name="model"):
    """학습된 모델로 에피소드를 실행하고 경로 데이터를 수집 (TP/FP 분리)"""
    print(f"\n{'='*60}")
    ...
    for episode in range(n_episodes):
        ...
        if hasattr(actual_env, 'drone'):
            ...
            if drone is not None and hasattr(drone, 'position'):
                ...
                episode_data['start_position'] = start_pos
        ...
        while not done:
            ...
            # ========== 액션 변환 추가 (array -> scalar) ==========
            if isinstance(action, np.ndarray):
                if action.ndim == 0:  # 0차원 배열
                    action = int(action)
                elif action.size == 1:  # 1개 요소
        ...
        if len(episode_data['flight_path']) > 0:
            episode_data['end_position'] = episode_data['flight_path'][-1]
        ...
        if drone is not None:
            ...
            if len(episode_data['detected_weeds']) == 0 and hasattr(drone, '_found_objects'):
        ...
        if len(episode_data['detected_weeds']) == 0:
            episode_data['detected_weeds'] = []

        # ===== 핵심 수정: TP/FP 분리 계산 =====
        tp_count, fp_count, fn_count = calculate_tp_fp(
            episode_data['detected_weeds'],
            episode_data['ground_truth_weeds']
        )

        found_percentage = tp_count / total_weeds if total_weeds > 0 else 0

        episode_data['metrics'] = {
            'path_length': total_steps,
            'true_positives': tp_count,      # 실제로 찾은 잡초
            'false_positives': fp_count,     # 오탐지
            'false_negatives': fn_count,     # 놓친 잡초
            'total_weeds': total_weeds,
            'found_percentage': found_percentage,
            'task_success': (tp_count == total_weeds)  # 100% 완전 탐지
        }
        # =======================================
        ...
        print(f"  True Positives (TP): {tp_count}/{total_weeds} ({found_percentage:.1%})")
        print(f"  False Positives (FP): {fp_count}")
        print(f"  False Negatives (FN): {fn_count}")
        print(f"  Task Success: {'✅ YES' if episode_data['metrics']['task_success'] else '❌ NO'}")
        print(f"  Start: {episode_data['start_position']}, End: {episode_data['end_position']}")
...
def plot_single_path(ax, data, title, path_color):
    ...
    if len(gt_weeds) > 0:
        ...
        if len(undetected) > 0:
            ...
            ax.scatter(undetected_arr[:, 0], undetected_arr[:, 1],
                      c='red', s=100, alpha=0.7, label='Missed (FN)',
                      edgecolors='darkred', linewidths=1.5, zorder=2, marker='x')

        # TP (찾은 잡초)
        if len(detected_arr) > 0:
            detected_arr = np.array(detected_arr)
            ax.scatter(detected_arr[:, 0], detected_arr[:, 1],
                      c='green', s=80, marker='o', label='Found (TP)',
    ...
    if len(flight_path) > 0:
        ax.plot(flight_path[:, 0], flight_path[:, 1],
               ...
               label='Flight path', zorder=4)

        # Start & End
        ax.scatter(flight_path[0, 0], flight_path[0, 1],
                  c='blue', s=250, marker='*', label='Start',
    ...
    success = '✅' if metrics['task_success'] else '❌'

    ax.set_title(f"{title}\nTP={tp}, FP={fp}, FN={fn} {success}",
...
def plot_overlay_comparison(ax, baseline_data, proposed_data):
    ...
    # Metrics
    baseline_tp = baseline_data['metrics']['true_positives']
    baseline_total = baseline_data['metrics']['total_weeds']
    proposed_tp = proposed_data['metrics']['true_positives']

    ax.set_title(f"Path Comparison\nBaseline: {baseline_tp}/{baseline_total}, Proposed: {proposed_tp}/{baseline_total}",
                fontsize=11, fontweight='bold')
    ax.set_xlabel('X position (grid)', fontsize=10)
    ax.set_ylabel('Y position (grid)', fontsize=10)
...
def create_comparison_plot(baseline_episodes, proposed_episodes,
    ...
    fig.suptitle(f'UAV Path Planning Comparison - Episode {episode_idx + 1}',
...
def print_statistics(baseline_episodes, proposed_episodes):
    """여러 에피소드의 통계 요약 출력 (TP/FP 포함)"""
    print(f"\n{'='*70}")
    print("PERFORMANCE STATISTICS (TP/FP Separated)")
    print(f"{'='*70}")

    baseline_tp = [ep['metrics']['true_positives'] for ep in baseline_episodes]
    baseline_fp = [ep['metrics']['false_positives'] for ep in baseline_episodes]
    baseline_fn = [ep['metrics']['false_negatives'] for ep in baseline_episodes]
    baseline_found = [ep['metrics']['found_percentage'] for ep in baseline_episodes]
    baseline_success = [ep['metrics']['task_success'] for ep in baseline_episodes]

    proposed_tp = [ep['metrics']['true_positives'] for ep in proposed_episodes]
    proposed_fp = [ep['metrics']['false_positives'] for ep in proposed_episodes]
    proposed_fn = [ep['metrics']['false_negatives'] for ep in proposed_episodes]
    proposed_found = [ep['metrics']['found_percentage'] for ep in proposed_episodes]
    proposed_success = [ep['metrics']['task_success'] for ep in proposed_episodes]

    print(f"\n📊 Baseline (van Essen et al.):") 
    print(f"  True Positives (TP):  {np.mean(baseline_tp):.1f} ± {np.std(baseline_tp):.1f}")
    print(f"  False Positives (FP): {np.mean(baseline_fp):.1f} ± {np.std(baseline_fp):.1f}")
    print(f"  False Negatives (FN): {np.mean(baseline_fn):.1f} ± {np.std(baseline_fn):.1f}")
    print(f"  Detection Rate:       {np.mean(baseline_found):.1%} ± {np.std(baseline_found):.1%}")
    print(f"  Success Rate:         {np.mean(baseline_success):.1%} ({sum(baseline_success)}/{len(baseline_success)})")

    print(f"\n📊 Proposed (CBAM + Multi-scale):")
    print(f"  True Positives (TP):  {np.mean(proposed_tp):.1f} ± {np.std(proposed_tp):.1f}")
    print(f"  False Positives (FP): {np.mean(proposed_fp):.1f} ± {np.std(proposed_fp):.1f}")
    print(f"  False Negatives (FN): {np.mean(proposed_fn):.1f} ± {np.std(proposed_fn):.1f}")
    print(f"  Detection Rate:       {np.mean(proposed_found):.1%} ± {np.std(proposed_found):.1%}")
    print(f"  Success Rate:         {np.mean(proposed_success):.1%} ({sum(proposed_success)}/{len(proposed_success)})")

    print(f"\n✨ Improvement:")
    tp_diff = np.mean(proposed_tp) - np.mean(baseline_tp)
    fp_diff = np.mean(proposed_fp) - np.mean(baseline_fp)
    found_diff = (np.mean(proposed_found) - np.mean(baseline_found)) * 100
    success_diff = (np.mean(proposed_success) - np.mean(baseline_success)) * 100

    print(f"  TP difference:      {tp_diff:+.1f} weeds")
    print(f"  FP difference:      {fp_diff:+.1f} (lower is better)")
    print(f"  Detection Rate:     {found_diff:+.1f}%p")
    print(f"  Success Rate:       {success_diff:+.1f}%p")
    print(f"{'='*70}\n")

def main():
    """메인 실행 함수"""
    print("\n" + "="*70)
    print("UAV PATH PLANNING VISUALIZATION (TP/FP Separated)")
    ...
    baseline_data_path = os.path.join(CONFIG['results_dir'], 'baseline_data_v2.npy')
    proposed_data_path = os.path.join(CONFIG['results_dir'], 'proposed_data_v2.npy')

    # 캐시 삭제 (항상 새로 생성)
    regenerate = True
    if regenerate:
        print("\n💡 Regenerating all data with TP/FP separation...")
    ...
    for i in range(len(baseline_episodes)):
        plot_path = os.path.join(CONFIG['results_dir'],
                                f'comparison_episode_{i+1}_v2.png')
